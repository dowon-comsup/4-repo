...
## 도구
- **`judge_director`** — 입력 `registry_text`(등기부 원문 텍스트), `employment_text`(고용현황 원문 텍스트) → 임원별 판정.
  - OCR은 **Claude(클라이언트)가 담당**: 대화창에 등기부·고용현황 PDF를 올리면 Claude가 읽어 텍스트를 추출 → 도구에 전달. 서버에 API 키 불필요.
...
## 한계 (MVP)
- **`judge_director`(등기임원)만** 제공. 가족종사자·비등기임원(체크리스트 점수 기반)은 미포함 — 필요 시 추가.
- 공동대표 친족분석 미포함(판정 결과의 공동대표 메모 생략).
